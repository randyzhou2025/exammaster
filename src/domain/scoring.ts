import type { Question, ExamTemplateConfig } from "@/types/exam";
...
export function scoreForQuestion(
  ...
  const section = template.sections.find((s) => s.type === q.type);
...
export function totalScoreForPaper(
  ...
  for (const q of questions) {
    ...
    const { points } = scoreForQuestion(q, sel, template);
    score += points;
